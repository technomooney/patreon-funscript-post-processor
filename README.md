...
| `PIXELDRAIN_ALLOW_ANONYMOUS` | `false` | Allow pixeldrain downloads with no API key (or after a configured one is rejected and you decline the replacement-key prompt). Opt-in only — pixeldrain's anonymous tier is capped at ~6GB/day |
| `SKIP_TITLE_KEYWORDS` | (none) | Semicolon-separated, case-insensitive words/phrases; a folder whose name contains any of them is skipped entirely before downloading — e.g. for a creator's periodic vote/poll posts that never have a funscript |
